...
> Trovare e sostituire. Dimenticate `sed`. `sd` è più intuitivo.

---

# `sd`: Esempi

Sostituisci "vecchio" con "nuovo" in un file:
```bash
sd 'vecchio' 'nuovo' file.txt
```

Sostituisci in tutti i file markdown trovati da `fd`:
```bash
fd -e md --exec sd 'errore di battitura' 'correzione'
```

Usa regex per sostituire:
```bash
sd 'versione-\d+' 'versione-2' Cargo.toml
```
